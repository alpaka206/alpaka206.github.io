type Props = { imageUrl: string; title: string; onClick?: () => void; className?: string; };
...
export function FolderIcon({ imageUrl, title, onClick, className }: Props) {
  return (
    ...
    >
      <div className="size-20 md:size-24 shrink-0">
        <img
          src={imageUrl}
          alt={title}
      ...
      <span className="text-xs md:text-sm font-medium drop-shadow">{title}</span>
    </button>
  );
}
